...
const setOceanData = (coord, oceanFloor) => {
    ...
    // diagnonal line
    const movingDown = coord.y1 < coord.y2
    const movingRight = coord.x1 < coord.x2
    const steps = (Math.max(coord.x1, coord.x2) - Math.min(coord.x1, coord.x2)) + 1
    for (let index = 0; index < steps; index++) {
        const modifiedX = coord.x1 + (movingRight ? index : index * -1)
        const modifiedY = coord.y1 + (movingDown ? index : index * -1)
        oceanFloor[modifiedY][modifiedX]++
    }
}
